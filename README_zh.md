...
**MemoSight**（Memory-Foresight-Based Reasoning，记忆-前瞻推理）将**上下文压缩**与**多 token 预测（MTP）**统一用于思维链推理：压缩历史 token 以抑制 KV cache 增长，并并行预测未来 token 以加速解码，同时使推理精度接近 vanilla SFT 基线。
