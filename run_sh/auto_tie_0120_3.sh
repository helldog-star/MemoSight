...
# ==================== 模型1: mtp_aux_cross_attn_full_E_w1e-1 ====================

train_model "mtp_aux_cross_attn_full_E_w1e-1" "True" "2e-5" "aug-wo-pc" "configs/mtp_aux_cross_attn_full_E_w1e-1.json"
if [ $? -ne 0 ]; then
    echo "❌ mtp_aux_cross_attn_full_E_w1e-1训练失败，退出"
    exit 1
fi

inference_and_evaluate "mtp_aux_cross_attn_full_E_w1e-1" "anchor-thought" "inference"

# ==================== 模型2: mtp_aux_cross_attn_full_stopG_E_w1e-1 ====================
train_model "mtp_aux_cross_attn_full_stopG_E_w1e-1" "True" "2e-5" "aug-wo-pc" "configs/mtp_aux_cross_attn_full_stopG_E_w1e-1.json"
if [ $? -ne 0 ]; then
    echo "❌ mtp_aux_cross_attn_full_stopG_E_w1e-1训练失败，退出"
    exit 1
fi

inference_and_evaluate "mtp_aux_cross_attn_full_stopG_E_w1e-1" "anchor-thought" "inference"
