...
# ==================== 模型1: epl_mtp_aux_cross_attn_E_w1e-1 ====================
train_model "epl_mtp_aux_cross_attn_E_w1e-1" "True" "2e-5" "aug-wo-pc" "configs/mtp_aux_cross_attn_config_E_w1e-1.json"
if [ $? -ne 0 ]; then
    echo "❌ epl_mtp_aux_cross_attn_E_w1e-1训练失败，退出"
    exit 1
fi

inference_and_evaluate "epl_mtp_aux_cross_attn_E_w1e-1" "anchor-thought" "inference"

# ==================== 模型2: epl_mtp_aux_midlayer_cross_attn_E_w1e-1 ====================
train_model "epl_mtp_aux_midlayer_cross_attn_E_w1e-1" "True" "2e-5" "aug-wo-pc" "configs/mtp_aux_midlayer_cross_attn_config_E_w1e-1.json"
if [ $? -ne 0 ]; then
    echo "❌ epl_mtp_aux_midlayer_cross_attn_E_w1e-1训练失败，退出"
    exit 1
fi

inference_and_evaluate "epl_mtp_aux_midlayer_cross_attn_E_w1e-1" "anchor-thought" "inference"
