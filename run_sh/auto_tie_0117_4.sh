...
# ==================== 模型1: lightthinker_epl_mtp_cross_attn_stopG ====================
train_model "lightthinker_epl_mtp_cross_attn_stopG" "True" "2e-5" "aug-wo-pc" "configs/mtp_aux_cross_attn_stopG_config.json"
if [ $? -ne 0 ]; then
    echo "❌ lightthinker_epl_mtp_cross_attn_stopG训练失败，退出"
    exit 1
fi

inference_and_evaluate "lightthinker_epl_mtp_cross_attn_stopG" "anchor-thought" "inference"

# ==================== 模型2: lightthinker_epl_mtp_midlayer_cross_attn ====================
train_model "lightthinker_epl_mtp_midlayer_cross_attn" "True" "2e-5" "aug-wo-pc" "configs/mtp_aux_midlayer_cross_attn_config.json"
if [ $? -ne 0 ]; then
    echo "❌ lightthinker_epl_mtp_midlayer_cross_attn训练失败，退出"
    exit 1
fi

inference_and_evaluate "lightthinker_epl_mtp_midlayer_cross_attn" "anchor-thought" "inference"
