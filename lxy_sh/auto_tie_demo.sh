...
ROOT_DIR="/mnt/lxy/RRcot"  # 项目根目录
INFERENCE_ROOT_DIR="${ROOT_DIR}/LightThinker"  # 推理脚本使用的代码根目录

# 输出路径配置
OUTPUT_BASE_DIR="/mnt/lxy/RRcot/output"  # 所有输出（训练、推理）的基础目录

# 模型和Tokenizer路径配置
TOKENIZER_PATH="/mnt/lxy/hf_models/Qwen2.5-1.5B-Instruct"  # Tokenizer路径
...
CONDA_ENV_NAME="lightinfer"  # Conda环境名称
...
train_model "mtp_aux_cross_attn_config_E_w1e-1_init" "True" "2e-5" "aug-wo-pc" "configs/mtp_aux_cross_attn_config_E_w1e-1_init.json"
if [ $? -ne 0 ]; then
    echo "❌ mtp_aux_cross_attn_config_E_w1e-1_init训练失败，退出"
    exit 1
fi

inference_and_evaluate "mtp_aux_cross_attn_config_E_w1e-1_init" "anchor-thought" "inference"
