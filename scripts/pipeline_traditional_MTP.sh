...
print_help() {
    cat <<'EOF'
统一流程脚本（Traditional MTP）：训练 / 推理 / 评估 / 全流程
与 scripts/pipeline.sh 行为一致，训练阶段使用 LightThinker/train_traditional_MTP.py（启动训练时会打印 MTP 模块额外参数量）。
产物统一保存于: <output_base_dir>/<exp_tag>/

用法:
  bash scripts/pipeline_traditional_MTP.sh --stage <train|infer|eval|all> [选项]
...
save_param_snapshot() {
    local snapshot="${EXP_ROOT}/run_${RUN_TS}.txt"
    {
        ...
        echo "train_entry=LightThinker/train_traditional_MTP.py"
        echo "stage=${STAGE}"
...
run_train() {
    ...
    local train_py="${ROOT_DIR}/LightThinker/train_traditional_MTP.py"
    ...
    log "开始训练（train_traditional_MTP.py，日志含 MTP 额外参数统计）..."
...
# # 运行示例 train
# bash /mnt/lxy/RRcot/scripts/pipeline_traditional_MTP.sh \
#   --stage train \
