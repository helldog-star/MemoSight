...
def plot_professional_tradeoff():
    # --- 1. 数据准备 ---
    # 压缩档位
    comp_levels = np.array(['d1', 'd2', 'd3', 'd4'])
    x = np.arange(len(comp_levels))
    # 对应的准确率数据
    accuracy = np.array([64.94, 66.19, 66.08, 65.01])
    # 对应的 Peak(Token) 指标 (模拟数据，用于柱状图)
    peak_token = np.array([1357, 1295, 1210, 1186])
    ...
    # --- 5. 坐标轴美化 ---
    ax1.set_xlabel('Offset Level', fontsize=12, fontweight='bold')
    ...
    plt.title('Impact of Model Offset on Accuracy and Speed', 
              fontsize=16, fontweight='bold', pad=20)
    
    plt.figtext(0.5, 0.01, "(b) Trade-off visualization across offset techniques", 
    ...
    # 保存两个格式
    plt.savefig("offset_ratio.pdf", format='pdf', bbox_inches='tight')
    plt.savefig("offset_ratio.png", format='png', dpi=300, bbox_inches='tight')
    
    print("已生成：offset_ratio.pdf 和 offset_ratio.png")
    plt.show()
